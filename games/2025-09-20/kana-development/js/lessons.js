...
(() => {
  ...
  // ========= Part 1 (Preview for ALL lessons) =========
  (function initPart1() {
    const part1 = $('#part-1');
    if (!part1) return;
    part1.innerHTML = '';
    ...
    // -3/-7 → pair cards; others → single-script cards
    ...
    // actions / Start button (wire after insertion)
  ...
  // ========= Part 2 (Identify) OR Typing for type-* =========
  (function initPart2() {
    const part2 = $('#part-2');
    if (!part2) return;

    if (IS_TYPING_ONLY) {
      // build typing scaffold directly in part 2
      part2.innerHTML = `
      ...
      `;
      initTyping(part2, GOAL_TYPE, true);
      return;
    }
    ...
    function nextQuestion(){
      ...
      const shuffledThemes=[...THEMES].sort(()=>Math.random()-.5);
  ...
  // ========= Part 3 (Typing) unless type-* used part 2 =========
  (function initPart3(){
  ...
  function initTyping(scopeEl,GOAL,isPart2){
    ...
    if(!wrapper){
      ...
      wrapper.className='type-glyph-wrapper';
    ...
    if(!input){
      ...
      input.setAttribute('placeholder','Type romaji…');
    ...
    if(!progressWrap){
      ...
      progressWrap.innerHTML='<div class="meter"><div class="meter-fill"></div></div><div class="meter-label"></div>';
